...
const Sync = {
  ...
  start() {
    this.fetch();
    this.timer = setInterval(() => this.fetch(), REFRESH_MS);
  },
...
const Render = {
  ...
  classes(rows) {
    ...
    order.forEach((cls, i) => {
      ...
      // Volunteer roster from PCO - split by service
      const volData = State.volunteers[cls];
      if (volData && (volData.first?.length || volData.second?.length)) {
        const section = document.createElement('div');
        section.className = 'vol-section';
        const renderRoster = (vols, label, labelClass) => {
        ...
        renderRoster(volData.first,  '1st', 'vol-svc-label-1');
        renderRoster(volData.second, '2nd', 'vol-svc-label-2');
...
(function init() {
  ...
  document.querySelector('.logo').addEventListener('click', () => {
    ...
    if (tapCount >= 3) { tapCount = 0; window.location.href = '/village-kids-dashboard/admin.html'; }
  });
