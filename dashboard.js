...
const Sync = {
  ...
  async fetch() {
    ...
    try {
      // Fetch Notion data and PCO data in parallel from browser
      const [data, volunteers, checkIns] = await Promise.all([
        VKShared.api.read(),
        VKShared.pco.fetchVolunteers().catch(() => ({})),
        VKShared.pco.fetchCheckIns().catch(() => null),
      ]);
