...
const Render = {
  ...
  _annList(items, elId) {
    ...
    el(elId).innerHTML = items.length
      ...
      : '<p class="ann-empty">All clear — have a great Sunday! ☀️</p>';
  },
  ...
  _schedRows(rows) {
    ...
    return rows.map((r, i) => {
      ...
      return '<div class="sched-row' + (isNow ? ' now' : '') + '">'
        ...
        + '<span class="sched-event">' + esc(r.event) + (isNow ? (function() {
        const rc = State.roomMode ? getRoomColor(State.roomMode) : { primary: 'var(--red)' };
        return '<span class="now-pill" style="background:' + rc.primary + '">Now</span>';
      })() : '') + '</span>'
...
const ROOM_COLORS = {
  'Nursery':                  { primary: '#b87a8a', light: 'rgba(184,122,138,.12)', dark: '#8a4a5a' },
  'Toddler/Wobbler':          { primary: '#c49a4a', light: 'rgba(196,154,74,.12)',  dark: '#8a6a1a' },
  'Preschool':                { primary: '#8faa4a', light: 'rgba(143,170,74,.12)',  dark: '#5a7a1a' },
  'Kindergarten - 1st Grade': { primary: '#4a8fc4', light: 'rgba(74,143,196,.12)', dark: '#1a5a8a' },
  '2nd-3rd Grade':            { primary: '#4aaa9a', light: 'rgba(74,170,154,.12)', dark: '#1a6a5a' },
  '4th-6th Grade':            { primary: '#7a6ac4', light: 'rgba(122,106,196,.12)',dark: '#4a3a8a' },
};

function getRoomColor(room) {
  return ROOM_COLORS[room] || { primary: 'var(--tan)', light: 'var(--tan-l)', dark: 'var(--tan-d)' };
}


const GREETINGS = [
  { time: 'morning', text: "You showed up. Honestly, that's half the battle." },
  { time: 'morning', text: "Another Sunday. Another opportunity to explain why we don't lick the carpet." },
  { time: 'morning', text: "You are shaping the next generation. No pressure." },
  { time: 'morning', text: "Somewhere in this building, a child is already asking when snack time is." },
  { time: 'morning', text: "You chose kids ministry. That tells us everything we need to know about you." },
  { time: 'morning', text: "Not all heroes wear capes. Some wear name tags and carry wet wipes." },
  { time: 'morning', text: "The glitter from last week's craft is still in the carpet. It lives there now." },
  { time: 'morning', text: "Small humans, big eternal impact. That's the job." },
  { time: 'morning', text: "Someone in your room will ask to go to the bathroom exactly when the lesson gets good." },
  { time: 'morning', text: "Village Kids: where every Sunday is an adventure and nobody really knows where the scissors went." },
  { time: 'morning', text: "You were made for this. Even the chaotic parts." },
  { time: 'morning', text: "It's a good day to make a kid feel seen." },
  { time: 'morning', text: "Somewhere a child is about to learn something they'll carry for the rest of their life." },
  { time: 'morning', text: "The craft supplies are probably fine. Probably." },
  { time: 'morning', text: "Low glamour. High impact. That's kids ministry." },
  { time: 'morning', text: "Today will be messy and wonderful and worth it." },
  { time: 'morning', text: "The kids are coming whether you're ready or not. Good thing you are." },
  { time: 'morning', text: "You volunteered for this. On purpose. That's remarkable." },
  { time: 'morning', text: "Small room. Small humans. Eternal significance." },
  { time: 'late',    text: "These kids will not remember the lesson. They will remember you showed up." },
  { time: 'late',    text: "What you're doing today matters more than you know. Also, snacks are in the cabinet." },
  { time: 'late',    text: "Almost there. You've earned whatever you're doing this afternoon." },
  { time: 'second',  text: "Second service. You've done this before. You can do it again." },
  { time: 'second',  text: "There is no such thing as a perfect Sunday morning. There is only this one. Make it count." },
];
...
function getGreeting() {
  ...
  let pool;
  if (mins >= 11 * 60) {
    pool = GREETINGS.filter(g => g.time === 'second' || g.time === 'late');
  } else if (mins >= 10 * 60) {
    pool = GREETINGS.filter(g => g.time === 'late' || g.time === 'morning');
  } else {
    pool = GREETINGS.filter(g => g.time === 'morning');
  }
...
const PinPad = {
  ...
  open(onSuccess) {
    ...
    el('pinOverlay').classList.add('open');
  },
  ...
  cancel() {
    el('pinOverlay').classList.remove('open');
    this.value = '';
  },
  ...
  _render() {
    ...
    dots.forEach((d, i) => {
      d.classList.toggle('filled', i < this.value.length);
...
const RoomSelect = {
  ...
  applyAdmin() {
    ...
    const adminBar = document.getElementById('roomBar');
    adminBar.style.background = 'var(--dark)';
    adminBar.classList.add('visible');
    document.getElementById('roomBarName').textContent = '⚙️ Admin';
  ...
  apply(room) {
    ...
    roomBar.style.background = col.primary;
    roomBar.classList.add('visible');
