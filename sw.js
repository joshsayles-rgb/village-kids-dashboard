...
self.addEventListener('fetch', event => {
  ...
  // Admin shell should prefer fresh code to avoid stale control panel behavior
  if (url.pathname === '/village-kids-dashboard/admin.html' || url.pathname.endsWith('/vk-admin-v10.html')) {
    event.respondWith(networkFirst(event.request, SHELL_CACHE));
    return;
  }

  // App shell → stale-while-revalidate for fast dashboard loads
